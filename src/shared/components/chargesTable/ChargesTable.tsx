...
const ChargesTable: React.FC<IChargesTableProps> = ({ rows = [], resend }) => {
        ...
        <Table sx={{ minWidth: 500 }} aria-label="custom pagination table">
          <TableHead style={{ borderBottom: '2px solid #747272' }}>
            <TableRow>
              ...
              <TableCell><HeaderTitle>Status</HeaderTitle></TableCell>
              <TableCell><HeaderTitle>Ação</HeaderTitle></TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {(rowsPerPage > 0
              ...
              <TableRow key={row.id}>
                ...
                <TableCell style={{ width: 160, fontWeight: '400', color: '#747272' }}>
                  <span style={{ color: getStatusColor(row.statusCode), fontWeight: 'bold' }}>{getStatusDescription(row.statusCode)}</span>
                </TableCell>
                <TableCell style={{ width: 160, fontWeight: '400', color: '#747272' }}>
                  { getStatusDescription(row.statusCode) === 'Pago' ?   <><ChargeButton onClick={() => openInNewTab(row.content)}>
                            {user.loading ? 'Carregando' : 'Comprovante'}
                        </ChargeButton> </> : ( 
                  <><ConfirmationModal title='Deseja reenviar a cobrança?' subtitle='Iremos reenviar a mensagem via WhatsApp.' actionBtnTitle='Reenviar' onConfirm={() => resend(row.id)} icon={<SendIcon />}></ConfirmationModal><IconButton onClick={(event) => handleClickClipboard(event, row.content)}>
                      <ContentCopyIcon />
                    </IconButton></> )
                  }
